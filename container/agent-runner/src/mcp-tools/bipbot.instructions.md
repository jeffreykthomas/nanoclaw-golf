...
- Use `bipbot_enqueue_linear_comment` when the issue should receive a status, audit, clarification, or human-facing Linear reply.
...
For Linear ingress tasks, you own the Linear thread response. Before ending the task, post a concise Linear comment explaining what you queued, or ask the original poster for the missing details and wait for a future ingress event before queuing work.
