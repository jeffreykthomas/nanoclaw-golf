...
- **A repeatable operational procedure** (how to send a campaign, publish, deploy, report) → Record it as a learned skill in `/workspace/agent/.claude/skills/<name>/SKILL.md` — see `/skill-builder`. Edit directly, no approval needed; changes hot-reload mid-session.
