...
function factCategoryToSchemaCategories(category: ProfileFactCategory): string[] {
  switch (category) {
    case 'demographics':
    case 'location':
    ...
    case 'relationship':
      return ['social_style_relationship_patterns', 'attachment_intimacy_boundaries'];
    case 'goal':
    ...
    case 'communication':
      return ['communication_voice_expression', 'conflict_power_strategy', 'social_style_relationship_patterns'];
    case 'golf':
...
function buildCategoryEvidenceCount(schema: UserProfileSchema, facts: ProfileFact[]): Record<string, number> {
  const counts = schema.top_level_categories.reduce<Record<string, number>>((acc, category) => {
    acc[category.key] = 0;
    return acc;
  }, {});
  ...
  for (const category of schema.top_level_categories) {
    ...
    for (const field of category.fields) {
      ...
      if (value === null || value === undefined) {
        metadata[category.key][field.key] = previous?.[category.key]?.[field.key] ?? metadata[category.key][field.key];
        continue;
      }
...
function buildDerivedSummary(params: {
  ...
  const categoryEvidenceCount = buildCategoryEvidenceCount(params.schema, params.facts);
  ...
  return {
    populatedFieldCount,
    unknownFieldCount: countInventoryFields(params.schema) - populatedFieldCount,
    categoryEvidenceCount,
    topThemes: params.facts.slice(-6).map((fact) => fact.fact),
  };
}

function hasMeaningfulProfileSignal(message: string, responseText: string, facts: ProfileFact[]): boolean {
  return facts.length > 0 || message.trim().length >= 24 || responseText.trim().length >= 24;
}
...
async function updateProfileInternal(params: {
  ...
  if (!hasMeaningfulProfileSignal(params.message, params.responseText, newFacts)) return;
  ...
  const profile: UserProfileDocument = {
    ...
    inventory,
    field_metadata: buildFieldMetadata(schema, inventory, allFacts, existing?.field_metadata, interactionAt),
    derived: buildDerivedSummary({ schema, inventory, facts: allFacts }),
    notes: {
      ...(existing?.notes ?? { model_update_status: 'skipped' }),
      last_coach_session_id: params.coachSessionId,
      last_interaction_at: interactionAt,
      last_model_update_at: modelResult.status === 'updated' ? interactionAt : existing?.notes?.last_model_update_at,
      model_update_status: modelResult.status,
    },
  };

  await writeUserProfileDocument(userId, profile);
  const report = buildUserProfileReport({ schema, profile, facts: allFacts });
  await writeProfileReport(userId, renderUserProfileReport(report));
  await writeCompactProfileSummary(userId, buildCompactProfileSummary(report));
}
...
export function queueUserProfileUpdate(params: {
  ...
  const next = previous
    ...
    .catch((error) => {
      log.warn('profile update chain failed', { error, userId: key });
    });
  updateChains.set(key, next);
  return next;
}

export function detectProfileCommand(message: string): 'report' | 'summary' | null {
  const normalized = message.trim().toLowerCase();
  if (
    /(generate|show|give me|create).*(profile|inventory).*(report|summary)/i.test(normalized) ||
...
export async function getProfileCommandResponse(params: {
  ...
  const userId = String(params.userId);
  const [summary, report] = await Promise.all([readCompactProfileSummary(userId), readProfileReport(userId)]);

  if (params.command === 'summary' && summary) return summary;
...
export async function getLatestProfileSummary(userId: string): Promise<string | null> {
...
export async function getLatestCheckInContext(userId: string): Promise<string | null> {
  const facts = (await loadProfileFacts(String(userId), 60)).filter((fact) => fact.category !== 'other');
...
export async function getUserProfileInventoryView(userId: string): Promise<{
  ...
  const fieldMetadata = profile?.field_metadata ?? createEmptyFieldMetadata(schema);
