...
async function findExistingOpenPullRequest(repoUrl: string, jobId: string): Promise<ExistingPullRequest | null> {
  ...
  try {
    ...
    log.warn('Failed to check for existing BipBot pull requests', { err, jobId, repoUrl });
    return null;
  }
}
...
export async function handleBipbotGatewayIpc(
  ...
  if (!requestId) {
    log.warn('BipBot gateway IPC blocked: missing requestId', { type });
    return true;
  }
  ...
  if (result.success) {
    log.info('BipBot gateway request completed', { type, requestId, sourceGroup });
  } else {
    log.warn('BipBot gateway request failed', { type, requestId, sourceGroup, message: result.message });
  }
  return true;
}
