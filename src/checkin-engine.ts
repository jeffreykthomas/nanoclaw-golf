...
import { getLatestCheckInContext, getLatestProfileSummary } from './profile/service.js';
import { listUserProfileDocuments, writeUserProfileDocument } from './profile/store.js';
...
export function evaluateCheckInOpportunity(profile: UserProfileDocument, now = new Date()): CheckInDecision {
  const lastInteractionAt = profile.notes.last_interaction_at;
  if (!lastInteractionAt) {
    return { shouldSend: false, reason: 'no prior interaction', urgency: 'gentle' };
  }

  const nowIso = now.toISOString();
  const hour = currentHourInTimezone(now);
  if (!CHECKIN_ALLOWED_HOURS.includes(hour)) {
    return { shouldSend: false, reason: 'outside allowed hours', urgency: 'gentle' };
  }

  const hoursSinceChat = hoursBetween(lastInteractionAt, nowIso);
  if (hoursSinceChat < CHECKIN_MIN_HOURS_SINCE_CHAT) {
    return { shouldSend: false, reason: 'recent chat', urgency: 'gentle' };
  }

  if (profile.notes.last_checkin_at) {
    const hoursSinceCheckIn = hoursBetween(profile.notes.last_checkin_at, nowIso);
    if (hoursSinceCheckIn < CHECKIN_MIN_HOURS_SINCE_LAST_CHECKIN) {
      return { shouldSend: false, reason: 'recent check-in', urgency: 'gentle' };
    }
  }
...
export function buildCheckInMessage(params: {
  ...
  const summaryLine = params.summary ? params.summary.split('\n')[0]?.trim() : '';
  const opener =
    params.urgency === 'firm'
...
export function startAutoCheckInLoop(sendMessage: (text: string) => Promise<boolean>): void {
  if (!AUTO_CHECKINS_ENABLED) {
    log.info('Auto check-ins disabled');
    return;
  }
  if (!isTelegramMirrorEnabled()) {
    log.info('Auto check-ins skipped: Telegram mirror not configured');
    return;
  }

  const loop = async () => {
    try {
      const profiles = await listUserProfileDocuments();
      for (const profile of profiles) {
        ...
        profile.notes.last_checkin_at = new Date().toISOString();
        await writeUserProfileDocument(profile.user_id, profile);
        log.info('Automatic check-in sent', {
      ...
      log.warn('Auto check-in loop failed', { error });
    }
