...
function getSyncGroup(userId: number): SyncGroup {
...
async function runArccosSyncForTask(
  ...
  const result = await runAgentTask({
    ...
    timeoutMs: ARCCOS_SYNC_TIMEOUT_MS,
    channelType: 'internal-arccos',
    platformId: `arccos:${task.user_id}`,
  });

  const structured = extractStructuredPayload(result.text);
  if (!structured) throw new Error('arccos_structured_payload_missing');
  const parsed = SyncPayloadSchema.safeParse(structured);
  if (!parsed.success) {
    throw new Error(`arccos_payload_invalid:${parsed.error.issues.map((issue) => issue.message).join(',')}`);
  }
  return parsed.data;
}
...
export async function syncArccosOnce(options?: {
  ...
  try {
    ...
    log.warn('Arccos pending fetch failed', { err: error });
    summary.failed += 1;
    return summary;
  }

  if (tasks.length === 0) {
    log.info('Arccos pending empty');
    return summary;
  }

  for (const task of tasks) {
    summary.attempted += 1;
    const taskContext = { userId: task.user_id };
    try {
      ...
      if (payload.status === 'ok') {
        summary.succeeded += 1;
        log.info('Arccos sync ok', { ...taskContext, inserted, updated, rounds: payload.rounds.length });
      } else if (payload.status === 'partial') {
        summary.partial += 1;
        log.warn('Arccos sync partial', { ...taskContext, inserted, updated, warnings: payload.meta?.['warnings'] });
      } else {
        summary.failed += 1;
      ...
      log.warn('Arccos sync failed', { ...taskContext, err: error });
      try {
  ...
  log.info('Arccos sync finished', { summary });
  return summary;
}
...
export function startArccosSyncLoop(): void {
  if (!AUTO_ARCCOS_SYNC_ENABLED) {
    log.info('Arccos sync loop disabled');
    return;
  }
  if (!CLAW_SIBLING_TOKEN || !COACH_APP_URL) {
    log.warn('Arccos sync loop skipped: CLAW_SIBLING_TOKEN or COACH_APP_URL not configured');
    return;
  }
  ...
  const loop = async () => {
    ...
    try {
      ...
      log.warn('Arccos sync loop tick failed', { err: error });
    } finally {
      running = false;
      setTimeout(loop, ARCCOS_SYNC_LOOP_INTERVAL_MS).unref();
    }
...
export function triggerArccosSyncInBackground(options: {
  ...
  setImmediate(async () => {
    try {
      ...
      log.warn('Arccos background sync failed', { err: error, userId: options.userId });
    }
  });
}
