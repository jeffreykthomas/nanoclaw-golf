...
import { buildCheckInMessage, evaluateCheckInOpportunity } from './checkin-engine.js';
...
const baseProfile: UserProfileDocument = {
  ...
  notes: {
    last_coach_session_id: 1,
    last_interaction_at: '2026-03-25T15:00:00.000Z',
    model_update_status: 'skipped',
  },
};

describe('evaluateCheckInOpportunity', () => {
  it('sends during allowed hours after a long enough gap', () => {
    const decision = evaluateCheckInOpportunity(baseProfile, new Date('2026-03-26T17:00:00.000Z'));
  ...
  it('skips outside allowed hours', () => {
    const decision = evaluateCheckInOpportunity(baseProfile, new Date('2026-03-26T08:00:00.000Z'));
...
describe('buildCheckInMessage', () => {
  ...
  it('prefers recent context over the compact profile summary', () => {
    const message = buildCheckInMessage({
      summary: 'This profile currently has 12 populated fields. current golf goals: compete this weekend.',
      recentContext: 'Most recent signal (2026-04-21): golf - processed the club championship results',
      urgency: 'gentle',
    });
